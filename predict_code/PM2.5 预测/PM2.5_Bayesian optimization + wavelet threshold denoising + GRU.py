...
def f_nn(params):
    ...
    def wavelet_noising(new_df):
        ...
        w = pywt.Wavelet(params['pywt'])  #定义小波对象   db3
    ...
    for i in range(len(data_path)):
        ...
        s1 = np.array(s1)
        s1 = s1.reshape(2400,1)
        scaler1 = MinMaxScaler(feature_range=(0, 1))
        s1 = scaler1.fit_transform(s1)
        s1 = s1.reshape(100,24,1)   #这里改步长
        X = s1[:79,:,:]
        y = s1[1:80,:,0]
        X_val = s1[80:99,:,:]
        y_val = s1[81:100,:,0]
    ...
    plt.plot(x,real,color='red',label='真实值')
    plt.plot(x,predict,color='green',label='预测值')
    plt.legend()
    plt.ioff()
    plt.show()
